...
namespace Kydo
{
	...
	inline void CheckEc(cl_int ec, int ln)
	{
		if (ec)
			std::printf("OpenCL Error at Line %i: %s\n", ln, clGetErrorString(ec));
	}

	#define CHECK_EC(ec) CheckEc(ec, __LINE__)

	Renderer::Renderer(Window &win, const std::string &source)
		: wnd(&win), src(source)
	{
		cl_int ec;
		...
		std::printf("Using OpenCL Device '%s'", device.getInfo<CL_DEVICE_NAME>(&ec).c_str()); CHECK_EC(ec);
