...
namespace Kydo
{
		...
		: wnd(win), pixels(wnd->pixels)
	{
		...
		auto platform = cl::Platform::getDefault(&ec);
		CHECK_EC();
		auto device = cl::Device::getDefault(&ec);
		if (ec == CL_DEVICE_NOT_FOUND || ec == CL_DEVICE_NOT_AVAILABLE)
			return;
		CHECK_EC();

		cl::string deviceName = device.getInfo<CL_DEVICE_NAME>(&ec);
		CHECK_EC();
		std::printf("Using OpenCL Device: '%s'\n", deviceName.c_str());

		context = cl::Context(device);
		q = cl::CommandQueue(context, device, NULL, &ec);
		CHECK_EC();
	}
